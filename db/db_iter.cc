...
// Helper function to extract user key from vkey format
// vkey format: user_key + sequence(8 bytes) + sort_key(8 bytes)
inline Slice ExtractUserVKey(const Slice& vkey) {
  assert(vkey.size() >= 16);
  if (vkey.size() < 16) {
    return Slice();
  }
  return Slice(vkey.data(), vkey.size() - 16);
}

namespace {

// DBVIter: 专门用于 VIterator 的迭代器，处理 vkey 格式
// vkey 格式: user_key + sequence(8字节) + sort_key(8字节)
// 
// 主要区别于 DBIter：
// 1. ExtractUserKey -> ExtractUserVKey (去掉16字节而不是8字节)
// 2. ParseInternalKey -> ParseInternalVKey
// 3. value() 返回从 VFile 读取的实际数据
class DBVIter : public Iterator {
 public:
  // Which direction is the iterator currently moving?
  // (1) When moving forward, the internal iterator is positioned at
  //     the exact entry that yields this->key(), this->value()
  ...
  DBVIter(DBImpl* db, const Comparator* cmp, Iterator* iter, SequenceNumber s,
          uint32_t seed)
      : db_(db),
        user_comparator_(cmp),
        iter_(iter),
  ...
  DBVIter(const DBVIter&) = delete;
  DBVIter& operator=(const DBVIter&) = delete;

  virtual ~DBVIter() { delete iter_; }
  virtual bool Valid() const { return valid_; }
  
  virtual Slice key() const {
    assert(valid_);
    return (direction_ == kForward) ? ExtractUserVKey(iter_->key()) : saved_key_;
  }
...
inline bool DBVIter::ParseKey(ParsedInternalKey* ikey) {
  Slice k = iter_->key();
  ...
  // 关键修改：使用 ParseInternalVKey 而不是 ParseInternalKey
  if (!ParseInternalVKey(k, ikey)) {
    status_ = Status::Corruption("corrupted internal vkey in DBVIter");
    return false;
  } else {
    return true;
  }
...
void DBVIter::Next() {
  ...
  if (direction_ == kReverse) {  // Switch directions?
    ...
    // Store in saved_key_ the current key so we skip it below.
    SaveKey(ExtractUserVKey(iter_->key()), &saved_key_);
  }

  FindNextUserEntry(true, &saved_key_);
}

void DBVIter::FindNextUserEntry(bool skipping, std::string* skip) {
  // Loop until we hit an acceptable entry to yield
  assert(iter_->Valid());
  assert(direction_ == kForward);
  do {
    ...
    if (ParseKey(&ikey) && ikey.sequence <= sequence_) {
      switch (ikey.type) {
        ...
        case kTypeValue:
          if (skipping &&
              user_comparator_->Compare(ikey.user_key, *skip) == 0) {
            // Entry hidden: same user_key with older sequence
          } else {
            valid_ = true;
            saved_key_.clear();
            return;
...
void DBVIter::Prev() {
  ...
  if (direction_ == kForward) {  // Switch directions?
    ...
    while (true) {
      ...
      if (user_comparator_->Compare(ExtractUserVKey(iter_->key()), saved_key_) < 0) {
        break;
      }
    }
    direction_ = kReverse;
...
void DBVIter::FindPrevUserEntry() {
  assert(direction_ == kReverse);
...
void DBVIter::Seek(const Slice& target) {
  direction_ = kForward;
  ClearSavedValue();
  saved_key_.clear();
  AppendInternalKey(&saved_key_,
...
Iterator* NewDBVIterator(DBImpl* db, const Comparator* user_key_comparator,
                         Iterator* internal_iter, SequenceNumber sequence,
                         uint32_t seed) {
  return new DBVIter(db, user_key_comparator, internal_iter, sequence, seed);
}
